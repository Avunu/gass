...
export async function createForm(
  ...
  try {
    ...
    if (sharingOptions) {
      applySharing(form, sharingOptions);
    }
    ...
    updateModalWithSuccess(result);

    return result;
  } catch (error) {
    showErrorMessage(error);
    Logger.log(`Error creating form: ${error}`);
    throw new Error(`Failed to create form: ${error}`, { cause: error });
  } finally {
 ...
 */
function addFormField(form: GoogleAppsScript.Forms.Form, field: FormField): void {
  switch (field.type) {
    case "TEXT":
    case "EMAIL":
    case "URL": {
 ...
 */
function applySharing(form: GoogleAppsScript.Forms.Form, options: FormSharingOptions): void {
  if (options.editors) {
...
export function getFormResponses(formId: string): FormResponse[] {
  try {
    ...
    return responses.map((response) => ({
      ...
      answers: parseResponseAnswers(response),
    }));
  } catch (error) {
    Logger.log(`Error retrieving form responses: ${error}`);
    throw new Error(`Failed to retrieve form responses: ${error}`, { cause: error });
  }
}
 ...
 */
function parseResponseAnswers(response: GoogleAppsScript.Forms.FormResponse): {
 ...
 */
export function getFormStatistics(formId: string): FormStatistics {
  try {
    const form = FormApp.openById(formId);
    const responses = form.getResponses();
    ...
    throw new Error(`Failed to retrieve form statistics: ${error}`, { cause: error });
  }
}
...
export function deleteForm(formId: string): void {
  try {
    ...
    throw new Error(`Failed to delete form: ${error}`, { cause: error });
  }
}

function showProcessingModal(): void {
  const html = HtmlService.createHtmlOutputFromFile(`${TEMPLATE_DIR}/FormService-inprogress`);
...
function updateModalWithSuccess(result: FormCreationResult): void {
  const template = HtmlService.createTemplateFromFile(`${TEMPLATE_DIR}/FormService-success`);
