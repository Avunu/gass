...
let sheetCache = new Map<number, GoogleAppsScript.Spreadsheet.Sheet>();

export function getSheet(sheetId: number): GoogleAppsScript.Spreadsheet.Sheet {
...
function getLastRowNumber(sheet: GoogleAppsScript.Spreadsheet.Sheet): number {
...
export async function getAllRows(sheetId: number, startRow: number = 2): Promise<RowResult[]> {
  const sheet = getSheet(sheetId);
  const lastRow = getLastRowNumber(sheet);
  ...
  return values.map((row, index) => ({
    data: row.map((cell) => convertFromSheet(cell)),
    rowNumber: index + startRow,
  }));
}

function convertFromSheet(value: SheetValue): SheetValue {
...
function convertToSheet(value: SheetValue): SheetValue {
...
export async function appendRow(sheetId: number, rowData: SheetValue[]): Promise<void> {
  const sheet = getSheet(sheetId);
  sheet.appendRow(rowData.map((value) => convertToSheet(value)));
}

export async function appendRows(sheetId: number, rows: SheetValue[][]): Promise<void> {
  const sheet = getSheet(sheetId);
  const range = sheet.getRange(sheet.getLastRow() + 1, 1, rows.length, rows[0].length);
  range.setValues(rows.map((row) => row.map((value) => convertToSheet(value))));
}

export async function prependRows(sheetId: number, rows: SheetValue[][]): Promise<void> {
  const sheet = getSheet(sheetId);
...
export async function updateRow(sheetId: number, rowNumber: number, rowData: SheetValue[]): Promise<void> {
  const sheet = getSheet(sheetId);
  const range = sheet.getRange(rowNumber, 1, 1, rowData.length);
  range.setValues([rowData.map((value) => convertToSheet(value))]);
}

export async function updateRows(
  sheetId: number,
  updates: { row: number; values: SheetValue[] }[],
): Promise<void> {
  const sheet = getSheet(sheetId);
  ...
  for (const chunk of chunks) {
    ...
    range.setValues(chunk.map((u) => u.values.map((value) => convertToSheet(value))));
  }
}

export async function deleteRow(sheetId: number, rowNumber: number): Promise<void> {
  const sheet = getSheet(sheetId);
  sheet.deleteRow(rowNumber);
}

export async function replaceAllValues(sheetId: number, newValues: any[][]): Promise<void> {
  const sheet = getSheet(sheetId);
  ...
  if (numRows > 0 && numColumns > 0) {
    ...
    const paddedValues = newValues.map((row) => {
      ...
      return paddedRow.map((value) => convertToSheet(value));
    });
...
export async function sortSheet(sheetId: number, sortOrders: SortSpec[]): Promise<void> {
  const sheet = getSheet(sheetId);
...
export function clearFilterRow(sheetId: number, range: string): void {
  const sheet = getSheet(sheetId);
...
export function evaluateFilter(value: SheetValue, filterValue: FilterValue): boolean {
...
export async function getFilteredRows(
  ...
  if (sortInfo) {
    ...
    if (rangeFilter) {
      rowRange = await getSortedRangeFilteredRows(sheet, headerRow, lastRow, rangeFilter, sortInfo.ascending);
  ...
  for (const colIndex of uniqueFilterColumns) {
    ...
    filterValues[colIndex] = range.getValues().map((row) => convertFromSheet(row[0]));
  }
  ...
  for (const range of ranges) {
    ...
    for (let i = 0; i < values.length; i++) {
      matches.push({
        data: values[i].map((cell) => convertFromSheet(cell)),
...
function encodeStringToUint8(str: string): Uint8Array {
  return new Uint8Array([...String(str).toUpperCase()].map((c) => c.charCodeAt(0)));
}

async function getSortedRangeFilteredRows(
  ...
  if (isStringComparison) {
    if (typeof min === "string") min = encodeStringToUint8(min);
    if (typeof max === "string") max = encodeStringToUint8(max);
  }
  ...
  const values = sheet
    ...
    .map((row, index) => {
      ...
      if (isStringComparison && value !== null) {
        ...
        value = encodeStringToUint8(String(value));
  ...
  if (!isUpperBoundOnly) {
    minBound = findRangeBound(values, min, true, isStringComparison);
  ...
  if (!isLowerBoundOnly) {
    maxBound = findRangeBound(values, max, false, isStringComparison);
  }
...
function findRangeBound(
  ...
  while (left <= right) {
    ...
    const cmp = isStringComparison
      ? compareUint8Arrays(target as Uint8Array, values[mid].value as Uint8Array)
      : compareValues(target as SheetValue, values[mid].value as SheetValue);
...
function compareUint8Arrays(target: Uint8Array, value: Uint8Array): number {
...
function compareValues(a: SheetValue, b: SheetValue): number {
  if (a === b) return 0;
  if (a === null) return -1;
  if (b === null) return 1;
