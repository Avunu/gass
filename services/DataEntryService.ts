 ...
 */
let entryTypeRegistry: Map<string, EntryConstructor> = new Map();
 ...
 */
export function registerEntryType(name: string, EntryClass: EntryConstructor): void {
  entryTypeRegistry.set(name, EntryClass);
}
 ...
 * @param name - The name of the entry type
 */
function getEntryType(name: string): EntryConstructor | undefined {
  return entryTypeRegistry.get(name);
}
 ...
 */
async function fetchLinkOptions<T extends Entry>(
 ...
 */
async function prepareLinkOptions(metadata: IEntryMeta): Promise<{ [fieldName: string]: string[] }> {
  ...
  for (const [fieldName, relationship] of relationships.entries()) {
    ...
    const TargetClass = getEntryType(relationship.targetClass);
    if (TargetClass) {
      const options = await fetchLinkOptions(TargetClass as any, relationship.targetField);
      linkOptions[fieldName] = options;
    } else {
      Logger.log(
        `Warning: Target class ${relationship.targetClass} not found in registry for field ${fieldName}`,
      );
 ...
 */
export async function showAddEntryDialog<T extends Entry>(
  EntryClass: (new () => T) & { _meta: IEntryMeta },
): Promise<void> {
  if (!EntryClass._meta) {
    throw new Error(
      `Entry class ${EntryClass.name} does not have metadata. Please call loadMetadata() with JSON Schema metadata.`,
    );
  }

  // Fetch link options for Link/LinkArray fields
  const linkOptions = await prepareLinkOptions(EntryClass._meta);
 ...
 */
function getSelectedRowData(EntryClass: EntryConstructor): {
  row: number;
 ...
 */
export async function showEditEntryDialog<T extends Entry>(
  ...
  const { row, entryData } = getSelectedRowData(EntryClass);
 ...
 */
export async function saveEntryFromDialog(
  ...
  try {
    ...
    const EntryClass = getEntryType(entryTypeName);
    if (!EntryClass) {
    ...
    if (isEdit && rowNumber) {
      const sheet = SpreadsheetApp.getActiveSpreadsheet()
        .getSheets()
        .find((s) => s.getSheetId() === EntryClass._meta.sheetId);
      if (!sheet) {
...
export function showDataEntrySidebar(): void {
  ...
  const html = template.evaluate().setTitle("Data Entry").setWidth(350);
 ...
 */
export async function loadSidebarFormData(
  entryTypeName: string,
  mode: "new" | "edit",
): Promise<{
  ...
  if (mode === "edit") {
    ...
    const result = getSelectedRowData(EntryClass);
